...
export default function SECTION_Nav_Profile(){
    ...
    return (
        ...
        <Button variant="ghost" onClick={() => window.location.href = `/content`} size="icon" className="p-1">
          <Folder className="w-5 h-5" />
        </Button>
        <Button variant="ghost" size="icon" onClick={() => window.location.href = `/stores`} className="p-1">
          <Store className="w-5 h-5" />
        </Button>
        <Button variant="ghost" onClick={() => window.location.href = `/settings`} size="icon" className="p-1">
